...
/**\brief This class defines functionality to handle n-dimensional
 * initializer_lists.
 */
...
struct _ndinitlist {
  ...
  ///\brief Function that traverses an n-dimensional initializer_list and copies
  ///the content to a flat vector
  //
  // \param vals Copy values to this vector
  // \param p    Start at this offset into vals; p is incremented corresponding
  // to values copied

  template <typename I>
  static void __copy(const init_t &init, I &dst) {
    for (auto i : init) _ndinitlist<T, N - 1>::__copy(i, dst);
  }

  template <typename I>
  static void copy(const init_t &init, I dst) {
    I dst_ = dst;
    __copy(init, dst_);
  }
  ...
  ///\brief Given an initializer_list and a dimension tuple, validate that the
  ///initializer_lists shape matches the tuple
...
class ndarray {
 ...
 public:
  ...
  ndarray(const shape_t<N> &shape,
          const std::function<T(const shape_t<N> &)> &f);
  // construct from pointer
  ndarray(const_pointer vals, shape_t<N> shape);
  ...
  // construct from values from some iterable type (ndarray, std::vector,
  // std::list etc.)
  template <typename Iterable>
  ndarray(const shape_t<N> &shape,
          const typename const_iterable_t<Iterable>::t &that);
  ...
  reference operator()(Ts... idxs) {
    return data[shape.tolinear(idxs...)];
  }
  template <typename... Ts>
  ...
  reference operator[](const shape_t<N> &idx) {
    return data[shape.tolinear(idx)];
  }
  const_reference operator[](const shape_t<N> &idx) const {
    return data[shape.tolinear(idx)];
  }
...
ndarray<T, N, Alloc>::ndarray(shape_t<N> shape)
  ...
  for (size_t i = 0; i < shape.size; ++i) new (data + i) T();
}

///\brief Array filled with given value
template <typename T, int N, typename Alloc>
ndarray<T, N, Alloc>::ndarray(shape_t<N> shape, T value) : ndarray(shape) {
  std::fill(data, data + shape.size, value);
}

///\brief Array filled using a function taking the linear index as argument
template <typename T, int N, typename Alloc>
ndarray<T, N, Alloc>::ndarray(const shape_t<N> &shape,
                              const std::function<T(ptrdiff_t)> &f)
    : ndarray(shape) {
  for (size_t i = 0; i < shape.size; ++i) data[i] = f(i);
}
...
ndarray<T, N, Alloc>::ndarray(const shape_t<N> &shape,
    ...
    : ndarray(shape) {
  auto it = shape.begin();
  for (size_t i = 0; i < shape.size; ++i, ++it) data[i] = f(*it);
}

// construct from pointer
template <typename T, int N, typename Alloc>
ndarray<T, N, Alloc>::ndarray(const_pointer vals, shape_t<N> shape)
    : ndarray(shape) {
...
template <typename T, int N, typename Alloc>
template <typename Iterator>
ndarray<T, N, Alloc>::ndarray(const shape_t<N> &shape, Iterator begin,
                              Iterator end)
    : ndarray(shape) {
  if (end - begin != (int) shape.size)
    throw ArrayInitializerException();
...
ndarray<T, N, Alloc>::ndarray(
    const shape_t<N> &shape, const typename const_iterable_t<Iterable>::t &that)
    : ndarray(shape, that.begin(), that.end()) {}

template <typename T, int N, typename Alloc>
template <typename Iterable>
ndarray<T, N, Alloc>::ndarray(const shape_t<N> &shape,
                              const const_iterable_t<Iterable> &that)
    : ndarray(shape, that.itable.begin(), that.itable.end()) {}
...
// move constructor
template <typename T, int N, typename Alloc>
ndarray<T, N, Alloc>::ndarray(ndarray<T, N, Alloc> &&that)
    : shape(that.shape), data(that.data) {
  that.data = nullptr;
}
...
ndarray<T, N, Alloc>::~ndarray() {
  if (data) {
    for (size_t i = 0; i < shape.size; ++i) (data + i)->~T();
...
void ndarray<T, N, Alloc>::to_stream(std::ostream &os) const {
  ...
  if (shape.size > 0) {
    os << data[0];
    for (size_t i = 1; i < shape.size; ++i) os << "," << data[i];
  }
  os << "]";
}

template <typename T, int N = 1>
ndarray<T, N> make_array(const typename _ndinitlist<T, N>::init_t &init) {
  return ndarray<T, N>(init);
}

template <typename T, int N = 1>
ndarray<T, N> new_array(const typename _ndinitlist<T, N>::init_t &init) {
  return new ndarray<T, N>(init);
}

template <typename T, int N = 1>
std::shared_ptr<ndarray<T, N>> new_array_ptr(
    typename _ndinitlist<T, N>::init_t init) {
  return std::shared_ptr<ndarray<T, N>>(new ndarray<T, N>(init));
}

template <typename T, int N = 1>
std::shared_ptr<ndarray<T, N>> new_array_ptr(shape_t<N> sz) {
  return std::shared_ptr<ndarray<T, N>>(new ndarray<T, N>(sz));
}
...
template <typename T>
std::shared_ptr<ndarray<T, 2>> new_array_ptr(
  ...
  return std::make_shared<ndarray<T, 2>>(
      shape(n, m), std::function<T(const shape_t<2> &)>(
                       [X](const shape_t<2> &i) { return X[i[0]][i[1]]; }));
}
...
template <typename T, int N, typename Alloc>
std::ostream &operator<<(std::ostream &os, const ndarray<T, N, Alloc> &a) {
  a.to_stream(os);
  return os;
}
}  // namespace monty
