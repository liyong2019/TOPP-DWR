 ...
 */
const char *ECOS_ver(void) { return thisVersion; }
...
/*
 * This function is reponsible for checking the exit/convergence conditions of
 * ECOS. If one of the exit conditions is met, ECOS displays an exit message and
 * returns the corresponding exit code. The calling function must then make sure
 * that ECOS is indeed correctly exited, so a call to this function should
 * always be followed by a break statement.
 *
 *    If mode == 0, normal precisions are checked.
 *
 *    If mode != 0, reduced precisions are checked, and the exit display is
 * augmented by "Close to". The exitcodes returned are increased by the value of
 * mode.
 *
 * The primal and dual infeasibility flags w->info->pinf and w->info->dinf are
 * raised according to the outcome of the test.
 *
 * If none of the exit tests are met, the function returns
 * ECOS_NOT_CONVERGED_YET. This should not be an exitflag that is ever returned
...
idxint checkExitConditions(pwork *w, idxint mode) {
  ...
  if ((-w->cx > 0 || -w->by - w->hz >= -abstol) &&
    ...
    if (w->stgs->verbose) {
      if (mode == 0) {
        ...
        PRINTTEXT("\nClose to OPTIMAL (within feastol=%3.1e, reltol=%3.1e, "
                  "abstol=%3.1e).",
                  MAX(w->info->dres, w->info->pres), w->info->relgap,
                  w->info->gap);
      }
    }
#endif
  ...
  /* Dual infeasible? */
  else if ((w->info->dinfres != ECOS_NAN) && (w->info->dinfres < feastol) &&
           (w->tau < w->kap)) {
#if PRINTLEVEL > 0
    if (w->stgs->verbose) {
      if (mode == 0) {
        ...
        PRINTTEXT("\nClose to UNBOUNDED (within feastol=%3.1e).",
                  w->info->dinfres);
      }
    }
#endif
  ...
  /* Primal infeasible? */
  else if (((w->info->pinfres != ECOS_NAN && w->info->pinfres < feastol) &&
            (w->tau < w->kap)) ||
           (w->tau < w->stgs->feastol && w->kap < w->stgs->feastol &&
            w->info->pinfres < w->stgs->feastol)) {
#if PRINTLEVEL > 0
    if (w->stgs->verbose) {
      if (mode == 0) {
        PRINTTEXT("\nPRIMAL INFEASIBLE (within feastol=%3.1e).",
                  w->info->pinfres);
      } else {
        PRINTTEXT("\nClose to PRIMAL INFEASIBLE (within feastol=%3.1e).",
                  w->info->pinfres);
      }
    }
#endif
...
idxint init(pwork *w) {
  ...
  for (i = 0; i < w->p; i++) {
    w->KKT->RHS1[w->KKT->Pinv[k++]] = w->b[i];
  }
  for (i = 0; i < w->C->lpc->p; i++) {
    w->KKT->RHS1[w->KKT->Pinv[k++]] = w->h[i];
    j++;
  }
  for (l = 0; l < w->C->nsoc; l++) {
    for (i = 0; i < w->C->soc[l].p; i++) {
      w->KKT->RHS1[w->KKT->Pinv[k++]] = w->h[j++];
    }
#if CONEMODE == 0
    w->KKT->RHS1[w->KKT->Pinv[k++]] = 0;
    w->KKT->RHS1[w->KKT->Pinv[k++]] = 0;
  ...
  /* initialize RHS2 */
  for (i = 0; i < w->n; i++) {
    w->KKT->RHS2[w->KKT->Pinv[i]] = -w->c[i];
  }
  for (i = w->n; i < w->KKT->PKPt->n; i++) {
    w->KKT->RHS2[w->KKT->Pinv[i]] = 0;
  }
  ...
  if (w->C->nexc == 0) {
    ...
    KKT_FACTOR_RETURN_CODE =
        kkt_factor(w->KKT, w->stgs->eps, w->stgs->delta, &w->info->tfactor_t1,
                   &w->info->tfactor_t2);
...
#endif
    w->info->nitref1 =
        kkt_solve(w->KKT, w->A, w->G, w->KKT->RHS1, w->KKT->dx1, w->KKT->dy1,
                  w->KKT->dz1, w->n, w->p, w->m, w->C, 1, w->stgs->nitref);
#if PROFILING > 1
...
#endif
    w->info->nitref2 =
        kkt_solve(w->KKT, w->A, w->G, w->KKT->RHS2, w->KKT->dx2, w->KKT->dy2,
                  w->KKT->dz2, w->n, w->p, w->m, w->C, 1, w->stgs->nitref);
#if PROFILING > 1
    w->info->tkktsolve += toc(&tkktsolve);
#endif
...
void updateStatistics(pwork *w) {
   ...
   * info->pinfres = w->hz + w->by < 0 ? w->hresx / w->resx0 / (-w->hz - w->by)
   * : NAN; info->dinfres = w->cx < 0 ? MAX(w->hresy/w->resy0,
   * w->hresz/w->resz0) / (-w->cx) : NAN;
   */
  info->pinfres = (w->hz + w->by) / MAX(w->ny + w->nz, 1) < -w->stgs->reltol
                      ? w->hresx / MAX(w->ny + w->nz, 1)
                      : ECOS_NAN;
  info->dinfres =
      w->cx / MAX(w->nx, 1) < -w->stgs->reltol
          ? MAX(w->hresy / MAX(w->nx, 1), w->hresz / MAX(w->nx + w->ns, 1))
          : ECOS_NAN;

#if PRINTLEVEL > 2
  PRINTTEXT("TAU=%6.4e  KAP=%6.4e  PINFRES=%6.4e  DINFRES=%6.4e\n", w->tau,
            w->kap, info->pinfres, info->dinfres);
#endif
}
...
void printProgress(stats *info) {
  if (info->iter == 0) {
...
#if PRINTLEVEL == 2
    PRINTTEXT("\nECOS %s - (C) embotech GmbH, Zurich Switzerland, 2012-15. "
              "Web: www.embotech.com/ECOS\n",
              ECOS_VERSION);
#if PRINTLEVEL > 2
    PRINTTEXT("Contributors: Core interior point solver:         A. Domahidi, "
              "embotech GmbH\n");
    PRINTTEXT("              Python interface and equilibration: Eric Chu, "
              "Stanford University\n");
    PRINTTEXT("              Branch-and-bound module:            Han Wang, "
              "Stanford University\n");
    PRINTTEXT("              Maths and Methods:                  Stephen Boyd, "
              "Stanford University\n");
#ifdef EXPCONE
    PRINTTEXT("              Exponential cone support:           Santiago "
              "Akle, Stanford University\n");
#endif
#endif
    PRINTTEXT("\n");
#endif
...
#if defined EXPCONE
    PRINTTEXT("It     pcost       dcost      gap   pres   dres    k/t    mu    "
              " step   sigma     IR    |   BT\n");
    PRINTTEXT("%2d  %+5.3e  %+5.3e  %+2.0e  %2.0e  %2.0e  %2.0e  %2.0e    ---  "
              "  ---   %2d %2d  - |  -  - \n",
              (int)info->iter, info->pcost, info->dcost, info->gap, info->pres,
              info->dres, info->kapovert, info->mu, (int)info->nitref1,
              (int)info->nitref2);
#else
    PRINTTEXT("It     pcost       dcost      gap   pres   dres    k/t    mu    "
              " step   sigma     IR\n");
    ...
    PRINTTEXT("%2d  %+5.3e  %+5.3e  %+2.0e  %2.0e  %2.0e  %2.0e  %2.0e    ---  "
              ...
              info->dres, info->kapovert, info->mu, (int)info->nitref1,
              (int)info->nitref2);
#endif /*End if defined EXPCONE for the branch of the first iteration non      \
          windows */
#endif
  } else {
#if defined _WIN32 || defined _WIN64
    ...
    PRINTTEXT("%2d  %+5.3e  %+5.3e  %+2.0e  %2.0e  %2.0e  %2.0e  %2.0e  %6.4f  "
              "%2.0e  %2d %2d %2d | %2d %2d\n",
              (int)info->iter, info->pcost, info->dcost, info->gap, info->pres,
              info->dres, info->kapovert, info->mu, info->step, info->sigma,
...
void RHS_affine(pwork *w) {
  ...
  for (i = 0; i < w->C->lpc->p; i++) {
    RHS[Pinv[j++]] = w->s[i] - w->rz[i];
  }
  k = w->C->lpc->p;
  for (l = 0; l < w->C->nsoc; l++) {
    for (i = 0; i < w->C->soc[l].p; i++) {
      RHS[Pinv[j++]] = w->s[k] - w->rz[k];
      k++;
    }
#if CONEMODE == 0
    RHS[Pinv[j++]] = 0;
...
void RHS_combined(pwork *w) {
  ...
  j = 0;
  for (i = 0; i < w->n; i++) {
    w->KKT->RHS2[Pinv[j++]] *= one_minus_sigma;
  }
  for (i = 0; i < w->p; i++) {
    w->KKT->RHS2[Pinv[j++]] *= one_minus_sigma;
  }
  for (i = 0; i < w->C->lpc->p; i++) {
    w->KKT->RHS2[Pinv[j++]] = -one_minus_sigma * w->rz[i] + ds1[i];
  }
  k = w->C->lpc->p;
  for (l = 0; l < w->C->nsoc; l++) {
    for (i = 0; i < w->C->soc[l].p; i++) {
...
pfloat expConeLineSearch(pwork *w, pfloat dtau, pfloat dkappa, idxint affine) {
  ...
  pfloat barrier =
      0.0; /* Variable to hold the value of f_e(s_e)+f^\star_e(z_e) */
    ...
    if (evalExpDualFeas(wz + w->C->fexv, w->C->nexc) == 1) {
      if (evalExpPrimalFeas(ws + w->C->fexv, w->C->nexc) == 1) {
        ...
        if (j == w->m) {
          ...
          barrier += evalSymmetricBarrierValue(
              ws, wz, tau + alpha * dtau, kap + alpha * dkappa, w->C, w->D);
 ...
 */
pfloat lineSearch(pfloat *lambda, pfloat *ds, pfloat *dz, pfloat tau,
                  pfloat dtau, pfloat kap, pfloat dkap, cone *C, kkt *KKT) {
  idxint i, j, cone_start, conesize;
  pfloat rhomin, sigmamin, alpha, lknorm2, lknorm, lknorminv, rhonorm,
      sigmanorm, conic_step, temp;
  ...
  for (i = 0; i < C->nsoc; i++) {
    ...
    for (j = 1; j < conesize; j++) {
      lkbar_times_dsk -= lkbar[j] * dsk[j];
    }
    lkbar_times_dzk = lkbar[0] * dzk[0];
    for (j = 1; j < conesize; j++) {
      lkbar_times_dzk -= lkbar[j] * dzk[j];
    }
    ...
    for (j = 1; j < conesize; j++) {
      rho[j] = lknorminv * (dsk[j] - factor * lkbar[j]);
    }
    ...
    for (j = 1; j < conesize; j++) {
      sigma[j] = lknorminv * (dzk[j] - factor * lkbar[j]);
    }
...
void backscale(pwork *w) {
  ...
  for (i = 0; i < w->n; i++) {
    printf("wx: %d, w->xequil[i]: %d,  w->tau: %d\n", w->x[i], w->xequil[i],
           w->tau);
    w->x[i] /= (w->xequil[i] * w->tau);
  }
  for (i = 0; i < w->p; i++) {
    w->y[i] /= (w->Aequil[i] * w->tau);
  }
  for (i = 0; i < w->m; i++) {
    w->z[i] /= (w->Gequil[i] * w->tau);
  }
  for (i = 0; i < w->m; i++) {
    w->s[i] *= (w->Gequil[i] / w->tau);
  }
  for (i = 0; i < w->n; i++) {
    w->c[i] *= w->xequil[i];
  }
...
idxint ECOS_solve(pwork *w) {
  ...
  for (w->info->iter = 0; w->info->iter <= w->stgs->maxit; w->info->iter++) {
    // printf("-----------loop count-------------\n");
    {
      ...
      // 将秒数转换为tm结构，以便获取小时、分钟、秒等信息
      tm_info = localtime(&tv.tv_sec);

      // 计算毫秒数（微秒转换为毫秒）
      int milliseconds = tv.tv_usec / 1000;

      // 打印当前时间的小时、分钟、秒、毫秒和循环次数
      printf("loop count: %d, current time: %02d:%02d:%02d:%03d\n",
             w->info->iter, tm_info->tm_hour, tm_info->tm_min, tm_info->tm_sec,
             milliseconds);
    }
    /* Compute residuals */
    computeResiduals(w);
     ...
     *
     * - the update was bad such that the primal residual PRES has increased by
     * a factor of SAFEGUARD, or
     * - the gap became negative
     *
     * If the safeguard is activated, the solver tests if reduced precision has
     * been reached, and reports accordingly. If not even reduced precision is
     * reached, ECOS returns the flag ECOS_NUMERICS.
     */
    if (w->info->iter > 0 &&
        (w->info->pres > SAFEGUARD * pres_prev || w->info->gap < 0)) {
#if PRINTLEVEL > 1
      if (w->stgs->verbose)
        deleteLastProgressLine(w->info);
      if (w->stgs->verbose)
        PRINTTEXT("Unreliable search direction detected, recovering best "
                  "iterate (%i) and stopping.\n",
...
#if PRINTLEVEL > 0
        if (w->stgs->verbose)
          PRINTTEXT("\nNUMERICAL PROBLEMS (reached feastol=%3.1e, "
                    "reltol=%3.1e, abstol=%3.1e).",
                    MAX(w->info->dres, w->info->pres), w->info->relgap,
                    w->info->gap);
#endif
        break;
    ...
    if (exitcode == ECOS_NOT_CONVERGED_YET) {
      /*
       * Full precision has not been reached yet. Check for two more cases of
       * exit: (i) min step size, in which case we assume we won't make progress
       * any more, and (ii) maximum number of iterations reached If these two
       * are not fulfilled, another iteration will be made.
       */
      ...
      if (w->info->iter > 0 && w->info->step == STEPMIN * GAMMA) {
        ...
        if (w->stgs->verbose)
          deleteLastProgressLine(w->info);
        if (w->stgs->verbose)
          PRINTTEXT("No further progress possible, recovering best iterate "
                    "(%i) and stopping.",
                    (int)w->best_info->iter);
#endif
        restoreBestIterate(w);
      ...
      /* MAXIT reached? */
...
#if PRINTLEVEL > 0
        const char *what = interrupted ? "SIGINT intercepted"
        ...
        if (exitcode == ECOS_NOT_CONVERGED_YET) {
          ...
          if (w->stgs->verbose) {
            const char *what =
                interrupted ? "INTERRUPTED" : "RAN OUT OF ITERATIONS";
            PRINTTEXT(
                "\n%s (reached feastol=%3.1e, reltol=%3.1e, abstol=%3.1e).",
                what, MAX(w->info->dres, w->info->pres), w->info->relgap,
                w->info->gap);
          }
#endif
        }
    ...
    /* SAFEGUARD:
     * Check whether current iterate is worth keeping as the best solution so
     * far, before doing another iteration
     */
    if (w->info->iter == 0) {
    ...
    /* Compute scalings */
#ifdef EXPCONE
    if (updateScalings(w->C, w->s, w->z, w->lambda, w->info->mu) ==
        OUTSIDE_CONE) {
#else
    if (updateScalings(w->C, w->s, w->z, w->lambda) == OUTSIDE_CONE) {
      ...
      if (w->stgs->verbose)
        deleteLastProgressLine(w->info);
      if (w->stgs->verbose)
        PRINTTEXT("Slacks/multipliers leaving the cone, recovering best "
                  "iterate (%i) and stopping.\n",
                  (int)w->best_info->iter);
#endif
      restoreBestIterate(w);
    ...
    w->info->nitref1 =
        kkt_solve(w->KKT, w->A, w->G, w->KKT->RHS1, w->KKT->dx1, w->KKT->dy1,
                  w->KKT->dz1, w->n, w->p, w->m, w->C, 0, w->stgs->nitref);
#if PROFILING > 1
    w->info->tkktsolve += toc(&tkktsolve);
#endif
    ...
    w->info->nitref2 =
        kkt_solve(w->KKT, w->A, w->G, w->KKT->RHS2, w->KKT->dx2, w->KKT->dy2,
                  w->KKT->dz2, w->n, w->p, w->m, w->C, 0, w->stgs->nitref);
#if PROFILING > 1
    w->info->tkktsolve += toc(&tkktsolve);
#endif

    /* dtau_denom = kap/tau - (c'*x1 + by1 + h'*z1); */
    dtau_denom = w->kap / w->tau - eddot(w->n, w->c, w->KKT->dx1) -
                 eddot(w->p, w->b, w->KKT->dy1) -
                 eddot(w->m, w->h, w->KKT->dz1);

    /* dtauaff = (dt + c'*x2 + by2 + h'*z2) / dtau_denom; */
    dtauaff =
        (w->rt - w->kap + eddot(w->n, w->c, w->KKT->dx2) +
         eddot(w->p, w->b, w->KKT->dy2) + eddot(w->m, w->h, w->KKT->dz2)) /
        dtau_denom;
    ...
    for (i = 0; i < w->m; i++) {
      w->KKT->dz2[i] = w->KKT->dz2[i] + dtauaff * w->KKT->dz1[i];
    }
    scale(w->KKT->dz2, w->C, w->W_times_dzaff);

    /* W\dsaff = -W*dzaff -lambda; */
    for (i = 0; i < w->m; i++) {
      w->dsaff_by_W[i] = -w->W_times_dzaff[i] - w->lambda[i];
    }
    ...
    w->info->step_aff =
        lineSearch(w->lambda, w->dsaff_by_W, w->W_times_dzaff, w->tau, dtauaff,
                   w->kap, dkapaff, w->C, w->KKT);

#ifdef EXPCONE

    /* The present value of step_aff keeps the primal and dual symmetric slacks
     * close to the boundary Here w->W_times_dzaff contains dzaff for the
     * exponential cones However W\dsaff is not initialized for the exponential
     * cones. first we must calculate dsaff From \muH(x)dxaff + dsaff = -s we
     * set dsaff to be dsaff = -s - \mu H(z)dzaff
     */
    ...
    if (w->C->nexc > 0) {
      ...
      PRINTTEXT("Affine backtracking %i %i %i %i %i \n", (int)w->info->pob,
                (int)w->info->cb, (int)w->info->cob, (int)w->info->pb,
                (int)w->info->db);
#endif
    ...
    dtau =
        ((1 - sigma) * w->rt - bkap / w->tau + eddot(w->n, w->c, w->KKT->dx2) +
         eddot(w->p, w->b, w->KKT->dy2) + eddot(w->m, w->h, w->KKT->dz2)) /
        dtau_denom;

    /* dx = x2 + dtau*x1;     dy = y2 + dtau*y1;       dz = z2 + dtau*z1; */
    for (i = 0; i < w->n; i++) {
      w->KKT->dx2[i] += dtau * w->KKT->dx1[i];
    }
    for (i = 0; i < w->p; i++) {
      w->KKT->dy2[i] += dtau * w->KKT->dy1[i];
    }
    for (i = 0; i < w->m; i++) {
      w->KKT->dz2[i] += dtau * w->KKT->dz1[i];
    }
    ...
    scale(w->KKT->dz2, w->C, w->W_times_dzaff);
    for (i = 0; i < w->m; i++) {
      w->dsaff_by_W[i] = -(w->dsaff_by_W[i] + w->W_times_dzaff[i]);
    }
    ...
    w->info->step = lineSearch(w->lambda, w->dsaff_by_W, w->W_times_dzaff,
                               w->tau, dtau, w->kap, dkap, w->C, w->KKT) *
                    w->stgs->gamma;
    ...
    if (w->C->nexc > 0) {
      ...
      PRINTTEXT("Combined backtracking %i %i %i %i \n", (int)w->info->cb,
                (int)w->info->cob, (int)w->info->pb, (int)w->info->db);
#endif
      ...
      if (w->info->step == -1) {
        ...
        if (w->stgs->verbose) {
          ...
          PRINTTEXT("Combined line search failed, recovering best iterate (%i) "
                    "and stopping.\n",
                    (int)w->best_info->iter);
        }
#endif
    ...
    /* Update variables */
    for (i = 0; i < w->n; i++) {
      printf("wx: %d, step: %d,  dx: %d\n", w->x[i], w->info->step,
             w->KKT->dx2[i]);
      w->x[i] += w->info->step * w->KKT->dx2[i];
    }
    for (i = 0; i < w->p; i++) {
      w->y[i] += w->info->step * w->KKT->dy2[i];
    }
    for (i = 0; i < w->m; i++) {
      w->z[i] += w->info->step * w->KKT->dz2[i];
    }
    for (i = 0; i < w->m; i++) {
      w->s[i] += w->info->step * w->dsaff[i];
    }
 ...
 */
void ECOS_updateData(pwork *w, pfloat *Gpr, pfloat *Apr, pfloat *c, pfloat *h,
                     pfloat *b) {
  idxint k;

#if defined EQUILIBRATE && EQUILIBRATE > 0
  /* Only unequilibrate the old data if the new data is in a different location
   * in memory. */
  /* This is required for backward compatibility since existing code might need
   * this step. */
  if (((Gpr + w->G->nnz < w->G->pr) || (w->G->pr + w->G->nnz < Gpr)) &&
...
#if defined EQUILIBRATE && EQUILIBRATE > 0
  /* Here, we need to equilibrate unconditionally since all new data is
   * unequilibrated. */
  set_equilibration(w);
#endif
