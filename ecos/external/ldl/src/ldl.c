...
void LDL_symbolic2(
  ...
  Lp[0] = 0;
  for (k = 0; k < n; k++) { Lp[k + 1] = Lp[k] + Lnz[k]; }
}
...
LDL_int LDL_numeric2    /* returns n if successful, k if D (k,k) is zero */
    (LDL_int n,         /* A and L are n-by-n, where n >= 0 */
     ...
     LDL_int Sign[],    /* input of size n, sign vector of diagonal entries for
                           regularization */
  ...
  for (k = 0; k < n; k++) {
    ...
    *t2 += toc(&clock);
    ...
    (int)k, (int)Sign[k], (int)k, D[k], Sign[k]*delta); D[k] = Sign[k]*delta;
    }
     */
  }
...
void LDL_lsolve(
  ...
  for (j = 0; j < n; j++) {
    p2 = Lp[j + 1];
    for (p = Lp[j]; p < p2; p++) { X[Li[p]] -= Lx[p] * X[j]; }
  }
}
...
/* ========================================================================== */
//
void LDL_lsolve2(LDL_int n,    /* L is n-by-n, where n >= 0 */
...
void LDL_ltsolve(
  ...
  for (j = n - 1; j >= 0; j--) {
    p2 = Lp[j + 1];
    for (p = Lp[j]; p < p2; p++) { X[j] -= Lx[p] * X[Li[p]]; }
  }
}
